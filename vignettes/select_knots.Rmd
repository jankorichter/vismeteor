...
via `freq.quantile()` (see `?freq.quantile`): partition the observations,
...
```{r, echo=TRUE}
rates$q   <- vismeteor::freq.quantile(rates$freq, 12)   # >= 12 meteors per bin
rates$qsl <- with(rates,
...
into bins of fixed minimum meteor count using `freq.quantile()` and
...
rates$qres <- vismeteor::freq.quantile(rates$freq, 500)
...
plot(bins$sl, bins$ratio, pch = 19, col = "blue",
     ylim = range(c(bins$ratio - 2 * bins$se,
...
The same `freq.quantile()` helper drives both the candidate grid above
...
- `?freq.quantile` — quantile bins used here for the residual diagnostic.
