...
obs <- subset(
    obs,
    radiant_alt > 15 & moon_alt < 0 & sun_alt < -5 &
        sl_start >= 139.2 & sl_end <= 140.6
)

deg2rad <- \(x) x * pi / 180
obs$sl <- (obs$sl_start + obs$sl_end) / 2
obs$t <- with(obs, t_eff * sin(deg2rad(radiant_alt)) / f)
...
- **`radiant_alt > 15`** — when the radiant is low above the horizon
  the geometric correction `sin(radiant_alt)` becomes numerically
...
- **`sl = (sl_start + sl_end) / 2`** — VMDB observations are intervals;
  ...
  regressor.
- **`t = t_eff * sin(radiant_alt) / f`** is the central correction. It
  ...
  radiant in the zenith (`sin(radiant_alt)`) and a clear sky (`f`).
...
`t` is `t_eff` after the zenith (`sin(radiant_alt)`) and cloud-cover
(`/ f`) corrections.
