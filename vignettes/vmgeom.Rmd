...
result.vs <- with(magnitudes, {
    N <- sum(Freq)
    tm <- vmgeom_vst_from_magn(magn, lim.magn)
    ...
    dr_dtm <- vmgeom_vst_to_r(tm.mean, deriv.degree = 1L)
    d2r_dtm2 <- vmgeom_vst_to_r(tm.mean, deriv.degree = 2L)
...
r.means <- with(magnitudes, {
    ...
    replicate(50000, {
        ...
        d2r_ds2 <- vmgeom_vst_to_r(s.mean, deriv.degree = 2L)
