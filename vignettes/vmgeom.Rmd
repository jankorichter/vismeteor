...
result.vs <- with(magnitudes, {
    ...
    r.hat <- vmgeom_vst_to_r(tm.mean)
    dr_dtm <- vmgeom_vst_to_r(tm.mean, deriv_degree = 1L)
    d2r_dtm2 <- vmgeom_vst_to_r(tm.mean, deriv_degree = 2L)
...
r.means <- with(magnitudes, {
    ...
    replicate(50000, {
        ...
        r.hat <- vmgeom_vst_to_r(s.mean)
        d2r_ds2 <- vmgeom_vst_to_r(s.mean, deriv_degree = 2L)
...
```{r, echo=TRUE, results='hide'}
magnitudes$p <- with(magnitudes, dvmgeom(m = magn, lm = lim_magn, result.rate$mean))
```
...
magnitudes$p[idx] <- with(
    magnitudes[idx, ],
    pvmgeom(m = magn + 1L, lm = lim_magn, result.rate$mean, lower.tail = TRUE)
)
```
