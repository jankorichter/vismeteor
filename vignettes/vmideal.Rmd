...
tm.mean <- with(magnitudes, {
    N <- sum(Freq)
    tm <- vmideal_vst_from_magn(magn, lim.magn)
...
print(paste("mean psi:", vmideal_vst_to_psi(tm.mean$val, lim.magn.mean)))
```
...
```{r, echo=TRUE}
print(vmideal_vst_to_psi(qnorm(0.90, tm.mean$val, tm.mean$sd), lim.magn.mean))
```
...
psi.mean <- vmideal_vst_to_psi(tm.mean$val, lim.magn.mean)
