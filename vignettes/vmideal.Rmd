...
observations <- with(PER_2015_magn$observations, {
    idx <- !is.na(lim_magn) & sl.start > 135.81 & sl.end < 135.87
    data.frame(
        magn_id = magn_id[idx],
...
```{r, echo=TRUE}
lim_magn.mean <- with(magnitudes, {
    N <- sum(Freq)
    sum(Freq * lim_magn) / N
})
print(paste("lim_magn.mean:", lim_magn.mean))
print(paste("mean psi:", vmideal_vst_to_psi(tm.mean$val, lim_magn.mean)))
```

The mean limiting magnitude `lim_magn.mean` is required to estimate the location parameter $\psi$.
...
```{r, echo=TRUE}
print(vmideal_vst_to_psi(qnorm(0.90, tm.mean$val, tm.mean$sd), lim_magn.mean))
```
...
psi.mean <- vmideal_vst_to_psi(tm.mean$val, lim_magn.mean)
magnitudes$p <- with(magnitudes, dvmideal(m = magn, lm = lim_magn, psi.mean))
```
...
magnitudes$p[idx] <- with(
    magnitudes[idx, ],
    pvmideal(m = magn + 1L, lm = lim_magn, psi.mean, lower.tail = TRUE)
)
```
...
```{r, echo=TRUE}
magnitutes.observed <- xtabs(Freq ~ magn_id + magn, data = magnitudes)
...
```{r, echo=TRUE}
magnitutes.expected <- xtabs(p ~ magn_id + magn, data = magnitudes)
